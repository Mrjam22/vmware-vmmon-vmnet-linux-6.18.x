...
class VMwareWizard:
    ...
    def detect_installed_kernels(self) -> List[KernelInfo]:
        ...
        for kernel_dir in sorted(modules_dir.iterdir()):
            ...
            supported = (major == 6 and minor in [16, 17, 18])
    ...
    def run(self):
        ...
        try:
            ...
            self.ui.show_banner(
                "VMware Module Installation Wizard",
                "Automated Kernel Module Compilation for Linux 6.16/6.17/6.18",
                icon="⚙️"
            )
            ...
            if not supported_kernels:
                self.ui.show_error("No supported kernels (6.16.x, 6.17.x or 6.18.x) with headers found!")
